...
# ============================================================
st.set_page_config(
    page_title="SION Visual Field Progression",
    page_icon="👁️",
    layout="wide",
)
...
# ============================================================
# ORIGINAL-SION-STYLE PERSISTENCE / EVENT LOGIC
...
def detect_swin_progression(
    ...
    """
    Detect persistent SION events for one patient-eye.
...
def make_interactive_burden_figure(g: pd.DataFrame, baseline_vfs=BASELINE_VFS):
    """Interactive SWIN burden chart using Plotly.

    Hover shows Region, Age, and Burden. Clicking a legend item toggles its
    line off/on; double-clicking isolates one line.
    """
    fig = go.Figure()
    ...
    ymax = max(
        1.0,
        float(g[["Normal_weight", "Superior_burden", "Inferior_burden", "Whole_burden"]].max().max()) * 1.08,
    )

    fig.update_layout(
        title="Superior Inferior Other-field Normal (SION) Burden",
        xaxis_title="Age",
        yaxis_title="Archetypal weight / burden",
        yaxis=dict(range=[-0.08, ymax]),
        hovermode="closest",
        legend=dict(
            title="Region",
            orientation="h",
...
# STREAMLIT UI
# ============================================================
st.title("SION Visual Field Progression")
st.caption(
...
if run:
    try:
        ...
        st.error(f"SWIN processing failed: {exc}")
        st.stop()
